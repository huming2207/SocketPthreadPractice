...
bool data_list_enqueue(char *data)
{
  ...
  // Detect if data is too short or null
  if(!data) {
    fprintf(stderr, "enqueue: data corrupted!\n");
    return false;
  }

  new_node->data = calloc(strlen(data) + EXTRA_STRING_SPACES, sizeof(char));

  // Check the data malloc result
  if (!new_node->data) {
    fprintf(stderr, "data_list_enqueue: failed to initialise the node: %s\n", strerror(errno));
    return false;
  }
...
char *data_list_dequeue()
{
  ...
  if (data_node_count <= 0 || !first_node || !first_node->data) {
    return NULL;
  }
